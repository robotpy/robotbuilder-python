...
Usage: run upload.py, and it will upload to the bot
'''
...
class RobotCodeInstaller(object):
    ...
    def upload_directory( self, remote_root, local_root, recursive=True, verbose=False, skip_special=True ):
        ...
        for root, dirs, files in os.walk( '.' ):
            ...
            if skip_special:
                for d in dirs[:]:
                    if d in ['.svn', '.hg', '.git', '__pycache__']:
                        dirs.remove(d)
            ...
            for fn in files:
                ...
                if skip_special and (ext == '.pyc' or ext == '.pickle' or ext == '.swp'):
                    continue
